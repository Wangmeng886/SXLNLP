...
model = BertModel.from_pretrained(r"F:\NLP\code\study\week6\bert-base-chinese", return_dict=False)
n = 2                       # 输入最大句子个数
vocab = 21128               # 词表数目
...
# self_attention_out参数 其中 embedding_size * embedding_size + embedding_size + embedding_size是self输出的线性层参数/**注：此处为残差结构**/，embedding_size + embedding_size是layer_norm层参数
self_attention_out_parameters = embedding_size * embedding_size + embedding_size + embedding_size + embedding_size

# Feed Forward参数 其中embedding_size * hide_size + hide_size第一个线性层/**注：第一个线性层扩张维度**/，embedding_size * hide_size + embedding_size第二个线性层，/**注：第二个线性层缩减维度，回归到embedding维度**/
...
feed_forward_parameters = embedding_size * hide_size + hide_size + hide_size * embedding_size + embedding_size + embedding_size + embedding_size##/**注：所以我认为第二个线性层由原来的embedding_size * hide_size + embedding_size修改为hide_size * embedding_size + embedding_size更好**/

# pool_fc层参数/**注：此处也为残差结构**/
